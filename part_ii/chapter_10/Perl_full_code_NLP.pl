...
sub find_drugs {
    ...
    # if this section header starts with meds or medications and it's immediately below another header, then treat this as part of the previous section
...
if ($sect =~ /\^[[[]]]\^a-zA-Z\]\*med(ication)?(s)?/i && ($line_count ==
$sect_head_line_index+1) && ($type eq 'admission') && ($sect !\~
