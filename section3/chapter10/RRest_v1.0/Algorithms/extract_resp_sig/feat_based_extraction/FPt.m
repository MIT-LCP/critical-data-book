...
for subj = up.paramSet.subj_list
    ...
    if strcmp(option(1:3), 'ekg')
        rel_string = [option(1:3), up.paths.filenames.qrss];
    else
        rel_string = [option(1:3), up.paths.filenames.pulse_peaks];
    end
    ...
    for rel_var_name_no = 1 : length(rel_var_names)
        eval(['save_name = ''' option(1:3), up.paths.filenames.fid_pts, rel_var_names{rel_var_name_no}(4:end) ''';']);
        ...
        if strcmp(option(1:3), 'ppg')
            ...
            rel_name2 = [option(1:3) up.paths.filenames.elim_vhf];
