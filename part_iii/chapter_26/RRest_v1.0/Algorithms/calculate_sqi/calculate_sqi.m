...
for subj = up.paramSet.subj_list
    ...
    for sig = {'ppg', 'ekg'}
        ...
        rel_name1 = [sig{1,1}, up.paths.filenames.elim_vhf];
        ...
        if strcmp(sig{1,1}, 'ppg')
            rel_name2 = [sig{1,1}, up.paths.filenames.pulse_peaks, 'COr', up.paths.filenames.elim_vhf];
            threshold = 0.86;                                               % ppg cross-correlation threshold value (for sqi)
        else
            rel_name2 = [sig{1,1}, up.paths.filenames.qrss, up.paths.filenames.elim_vhf];
        ...
        eval(['sqi.' sig{1,1} ' = temp;']);  clear temp coeff HR SQI sig_data beats threshold win_no
